  ...
  const addToCalendar = async () => {
    try {
      ...
      const response = await fetch('http://20.216.143.86/event/add', {
        ...
        body: JSON.stringify({
          events: toti, // Corrected the property name from "event_" to "event"
        }),
      });
      // Assuming there is some code here to handle the response from the server
    } catch (error) {
    ...
    <div style={{ textAlign: 'center', margin: '0 auto', maxWidth: '900px', marginLeft: '500px' }}>
      ...
      >
        ...
        <div style={{ marginTop: '20px' }}>
          <button onClick={addToTimeline}>Ajoute cette activité à ton calendrier</button>
        </div>
